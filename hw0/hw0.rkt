...
; ********************************************************
; CS 201 HW #0  DUE Wednesday 9/10/2025, 11:59 pm
...
(define (kg2lb mass)
  (let* ((total-pounds (* mass 2.2))
         (whole-pounds (inexact->exact (floor total-pounds)))
         (ounces (inexact->exact (floor (* (- total-pounds whole-pounds) 16))))
         (pound-word (if (= whole-pounds 1) 'pound 'pounds))
         (ounce-word (if (= ounces 1) 'ounce 'ounces)))
    (list whole-pounds pound-word ounces ounce-word)))
...
; issues: June, July, August, September 2025 See http://cacm.acm.org/
